...
### Fixed
- Removed the legacy `SkipUserOOBE`/`SkipMachineOOBE` elements from the `OOBE` block in all three scripts (added in v1.3). These are deprecated Windows 7-era settings that are **not** in the modern Microsoft-Windows-Shell-Setup OOBE schema, so Windows 10/11 and Server 2022/2025 reject them at first boot with "Windows could not parse or process unattend answer file... for pass [oobeSystem]. A component or setting specified in the answer file does not exist." Confirmed on a real W11 clone (2026-07-16) - this was the first time any clone had actually OOBE-booted from the generated answer file, so the bad elements had never been exercised before. OOBE is still fully skipped by the remaining `Hide*` flags plus the declared `UserAccounts` (the modern mechanism); nothing replaces them. Verified the rest of the W11 OOBE block (`HideEULAPage`, `HideLocalAccountScreen`, `HideOEMRegistrationScreen`, `HideOnlineAccountScreens`, `HideWirelessSetupInOOBE`, `NetworkLocation`, `ProtectYourPC`) against the current schema - all valid. Only W11 is re-tested this cycle; RDSH/Server received the identical removal but remain OOBE-boot-untested.
