...
================================================================
 ...
 2. Open an elevated PowerShell prompt (Run as Administrator) on the
    ...
    to the logs under C:\ArcLogs\GoldImagePrep, then runs it:
       ...
       $p="$env:SystemDrive\ArcLogs\GoldImagePrep\Prep-WS2025-RDSH-Template.ps1"; md (Split-Path $p) -Force|Out-Null; irm https://raw.githubusercontent.com/Arc-hub-tech/Automations/develop/gold-image/Prep-WS2025-RDSH-Template.ps1 -OutFile $p; Set-ExecutionPolicy Bypass -Scope Process -Force; & $p

    Why download-then-run (not `irm | iex`): this is a two-stage build -
    stage 1 installs/configures then REBOOTS (required for the RDSH role +
    Defender removal); stage 2 resumes automatically at your next logon,
    re-sweeps appx, reminds you to enrol Sentinel + verify Office/Teams, and
    prompts before syspreping. The reboot/resume needs a real script file on
    disk to relaunch, which a piped `irm | iex` run does not have.
    (A local copy works too: just run `.\Prep-WS2025-RDSH-Template.ps1`.)
...
function Disable-VDIServices {
    # Overhead services to disable on a shared/non-persistent VM. Guarded, so a
    # SKU missing a service (e.g. Xbox on Server) is simply skipped. NB: leaves
    # WSearch/Spooler/UsoSvc/CDPSvc alone. SysMain is deliberately NOT here -
    # Windows actively restores it at servicing / clone first-boot, so the image
    # is the wrong layer to enforce it and the payoff on SSD-backed VDI is marginal
    # (enforce via GPO if ever needed). These services are not Windows-restored, so
    # a single stage-1 pass holds through the reboot.
    'WerSvc','MapsBroker','CscService','XblAuthManager','XblGameSave','XboxGipSvc','XboxNetApiSvc' |
...
# the answer file, then sets a temporary auto-logon and reboots (the reboot
# the RDSH role + Defender removal already require). A one-time logon-triggered
# scheduled task relaunches this script as stage 2 (-Resume), which re-sweeps
# appx and prompts before syspreping. Requires the download-and-run invocation
# (a real script file on disk) - it cannot resume an `irm | iex` piped run.
# ---------------------------------------------------------------
...
Write-Host ("Prep-WS2025-RDSH-Template.ps1  v{0}" -f $ScriptVersion) -ForegroundColor Cyan
...
if ($Resume) {
    ...
    Write-Host "  - Windows Defender was removed - install/enrol Sentinel now if this image needs EDR before it serves users." -ForegroundColor Yellow
    Write-Host "  - Verify Office / Teams launch." -ForegroundColor Yellow
    Write-Host "  (You can do those in another window - this prompt will wait.)" -ForegroundColor DarkGray
...
# Enable Remote Desktop - the standard LAN remote-admin path, and the whole point
# of a session-host image. fDenyTSConnections=0 is the "Allow remote connections to
# this computer" switch; then open the Remote Desktop firewall group so the listener
# is actually reachable. The group is enabled by its locale-independent resource
# string (@FirewallAPI.dll,-28752) rather than the English "Remote Desktop" display
# name, so it works on any OS language. NLA/TLS/high-encryption are enforced
# immediately below, so remote access is on but hardened.
...
# Services that are pure overhead on a shared/non-persistent VM.
Disable-VDIServices
...
Write-Host "`nStage 1 complete (RDSH role, installs, Defender removal, answer file, appx sweep, hardening)." -ForegroundColor Green
$resumeReady = Register-GoldImageResume
if ($resumeReady) {
    # Temporary one-shot auto-logon so the reboot resumes hands-free. Stage 2
    # scrubs it before the sysprep prompt. This reboot is the one the RDSH role
    # + Defender removal already require. Fall back to manual login if no admin.
    if ($StandingAdminReady) {
    ...
    Write-Host "Stage 2 re-sweeps appx, checks BitLocker, reminds you to enrol Sentinel + verify Office/Teams, then prompts before syspreping." -ForegroundColor Green
    Write-Host "If stage 2 doesn't auto-start, run:  & '$PSCommandPath' -Resume" -ForegroundColor DarkGray
    Write-Host "`nRebooting in 15 seconds (required for the RDSH role + Defender removal). Press Ctrl+C to cancel..." -ForegroundColor Yellow
    Stop-Transcript | Out-Null
    Start-Sleep -Seconds 15
    Restart-Computer -Force
} else {
    Write-Host "REBOOT this VM (required for RDSH role + Defender removal), then run the downloaded script again with -Resume." -ForegroundColor Yellow
    Write-Host "Before syspreping: install/enrol Sentinel and verify Office/Teams. Manual sysprep command (sysprep.exe is NOT on PATH):" -ForegroundColor Green
    Write-Host "  C:\Windows\System32\Sysprep\sysprep.exe /oobe /generalize /shutdown /unattend:C:\Windows\Panther\unattend.xml" -ForegroundColor Green
    Stop-Transcript | Out-Null
}
