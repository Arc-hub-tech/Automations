...
.SYNOPSIS
    ...
    3. Installs Microsoft 365 Apps (64-bit, Monthly Enterprise, Shared Computer Licensing for VDI)
    4. Installs new Teams machine-wide (with VDI/AVD optimisation reg key)
    5. Installs common apps (7-Zip, SumatraPDF) via winget
    6. Removes provisioned + installed bloatware appx packages
    7. Disables consumer content / suggested apps so clones stay clean
...
# ---------------------------------------------------------------
# 4. New Teams (machine-wide provisioning, VDI optimised)
# ---------------------------------------------------------------
...
if (Get-AppxProvisionedPackage -Online | Where-Object DisplayName -eq 'MSTeams') {
    ...
    Start-ProcessWithHeartbeat -FilePath $boot -ArgumentList "-p" -Label "Teams bootstrapper" | Out-Null   # -p provisions for all users
}
...
# ---------------------------------------------------------------
# 5. Common third-party apps via winget - no hardcoded download URLs
...
# ---------------------------------------------------------------
# 6. FSLogix agent - DORMANT install. Completely inert without the
...
# ---------------------------------------------------------------
# 7. Remove bloatware (provisioned = future users, installed = current)
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# 8. Stop Windows re-adding junk on new profiles
# ---------------------------------------------------------------
...
# ---------------------------------------------------------------
# 9. Sysprep-readiness sweep: remove appx packages that are installed
